...
namespace Aggregate.Persistence.Binary.Domain
{
    ...
    public class Order : IOrder
    {
        ...
        {
            if (quantity < 0) throw new OrderOperationException("Unable to add product with negative quantity.");
            if (quantity == 0) throw new OrderOperationException("Unable to add product with no quantity.");
        }

        private void ReCalculateTotalPrice()
        {
            if (_lines.Count == 0) TotalCost = 0;
